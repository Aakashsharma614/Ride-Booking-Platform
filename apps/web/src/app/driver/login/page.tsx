...
export default function DriverLogin() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { accessToken } = await api.driverLogin(email, password);
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center px-6 py-12 bg-black animate-fade-in">
      <div className="mb-10 text-center">
        <h1 className="text-4xl font-black text-white mb-2">Driver <span className="text-blue-400">Login</span></h1>
        <p className="text-gray-400 text-sm">Sign in to start earning</p>
      </div>
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <Button type="submit" className="w-full bg-blue-500 hover:bg-blue-600 text-white" size="lg" disabled={loading}>
      ...
      <p className="text-center text-sm text-gray-500 mt-8">
        New driver?{" "}
        <Link href="/driver/register" className="text-blue-400 font-medium hover:underline">Apply to drive</Link>
      </p>
    </div>
  );
}
